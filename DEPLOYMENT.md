...
- Создаст 5 тематических уровней (если БД пустая)
- Создаст админ-пользователя (из переменных `ADMIN_USERNAME` / `ADMIN_PASSWORD` в .env, по умолчанию admin/admin123)
...
- Admin Panel: http://localhost:5001 (admin / admin123)
...
- **Аналитика** - графики регистраций, игр, воронка уровней
- **Управление уровнями** - визуальный редактор уровней
- **Пользователи** - управление игроками
- **Экспорт** - CSV, Excel, JSON отчёты

### Публичная аналитика

Можно создать защищённую паролем ссылку на аналитику:

1. Админ-панель → Аналитика → "Поделиться"
2. Задайте пароль для публичного доступа
3. Скопируйте ссылку вида `/admin/public/{token}`
4. Получатель сможет смотреть статистику и скачивать PDF

### Уровни

При первом запуске создаются 5 тематических уровней:

| # | Название | Описание | Препятствия |
|---|----------|----------|-------------|
| 1 | Первый вкус | Лёгкий старт | Куриная ножка |
| 2 | Острые крылышки | Два крыла | Симметричные крылья |
| 3 | Золотое ведёрко | Собери вёдра | Форма ведра |
| 4 | Буква R | Для Rostic's | Буква R |
| 5 | Полковник | Финальный босс | Лицо в очках |

Уровни можно редактировать в конструкторе: Админ-панель → Конструктор уровней
...
docker-compose exec backend flask db revision -m "описание изменений"

# Статус миграций
docker-compose exec backend flask db current
```

Миграции автоматически применяются при запуске контейнера backend.
