...
CREATE TABLE IF NOT EXISTS levels (
    ...
    item_types VARCHAR(50)[] DEFAULT ARRAY['chicken', 'burger', 'fries', 'cola', 'bucket'],
...
VALUES
    ('Tutorial', 1, 7, 7, 20,
     ARRAY['chicken', 'burger', 'fries'],
     '{"collect": {"chicken": 5}, "min_score": 500}'::jsonb,
     '[]'::jsonb,
     TRUE),
    ('Easy Start', 2, 7, 7, 25,
     ARRAY['chicken', 'burger', 'fries', 'cola'],
     '{"collect": {"chicken": 10, "burger": 5}, "min_score": 1000}'::jsonb,
     '[]'::jsonb,
     TRUE),
    ('Combo Master', 3, 7, 7, 30,
     ARRAY['chicken', 'burger', 'fries', 'cola', 'bucket'],
     '{"collect": {"fries": 15}, "combos": {"4_match": 3}, "min_score": 2000}'::jsonb,
     '[]'::jsonb,
     TRUE),
    ('Big Order', 4, 7, 7, 35,
     ARRAY['chicken', 'burger', 'fries', 'cola', 'bucket'],
     '{"collect": {"chicken": 20, "burger": 15, "fries": 10}, "min_score": 3000}'::jsonb,
     '[{"row": 3, "col": 3}]'::jsonb,
     TRUE),
    ('Rush Hour', 5, 7, 7, 25,
     ARRAY['chicken', 'burger', 'fries', 'cola', 'bucket'],
     '{"collect": {"chicken": 25, "cola": 15}, "combos": {"5_match": 2}, "min_score": 5000}'::jsonb,
